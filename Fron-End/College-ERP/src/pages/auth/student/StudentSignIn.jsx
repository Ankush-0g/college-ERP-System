    ...
    try {
      ...
      if (response.ok) {
        ...
        if (studentResponse.ok) {
          ...
          if (storedCourse && !studentData.course) {
            studentData.course = storedCourse;
          }
          if (storedSemester && !studentData.semester) {
            studentData.semester = storedSemester;
          }
          if ((!studentData.subjects || studentData.subjects.length === 0) && storedSubjects) {
            try {
              const parsedSubjects = JSON.parse(storedSubjects);
              if (Array.isArray(parsedSubjects)) {
                studentData.subjects = parsedSubjects;
              }
            } catch (err) {
              console.warn("Unable to parse studentSubjects from localStorage", err);
            }
          }
          ...
          if (studentData.subjects) {
            localStorage.setItem("studentSubjects", JSON.stringify(studentData.subjects));
          }
